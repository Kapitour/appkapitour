    ...
    return (
      ...
      >
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, { fontSize: Math.round(16 * fontScale) }]}>Carregando produtos...</Text>
        </View>
      </LinearGradient>
    );
  }
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                <Text style={[styles.emptyText, { fontSize: Math.round(16 * fontScale) }]}>Nenhum produto encontrado</Text>
              </View>
            }
          />
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            {selectedProduct && (
              <>
                <Text style={[styles.modalTitle, { fontSize: Math.round(20 * fontScale) }]}>{selectedProduct.nome}</Text>
                <Image
                  source={{ 
                ...
                />
                <ScrollView>
                  <Text style={[styles.modalText, { fontSize: Math.round(14 * fontScale) }] }>
                    {selectedProduct.descricao}
                  </Text>
                  <Text style={[styles.modalText, { fontSize: Math.round(14 * fontScale) }]}>
                    <Text style={[styles.bold, { fontSize: Math.round(14 * fontScale) }]}>R$ </Text> {obterValor(selectedProduct.valor_unid)}
                  </Text>
                  <Text style={[styles.modalText, { fontSize: Math.round(14 * fontScale) }]}>
                    <Text style={[styles.bold, { fontSize: Math.round(14 * fontScale) }]}>Estoque:</Text> {verificarEstoque(selectedProduct.id)} unidades
                  </Text>
                </ScrollView>
                <TouchableOpacity 
                ...
                >
                  <Text style={[styles.buyText, { fontSize: Math.round(16 * fontScale) }, verificarEstoque(selectedProduct.id) === 0 && styles.buyTextDisabled]}>
                    {verificarEstoque(selectedProduct.id) > 0 ? 'Comprar' : 'Indisponível'}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={closeModal}>
                  <Text style={[styles.closeText, { fontSize: Math.round(16 * fontScale) }]}>Fechar</Text>
                </TouchableOpacity>
              </>
            )}
          </View>
