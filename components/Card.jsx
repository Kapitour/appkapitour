  ...
  return (
    ...
    >
      ...
      <View style={{ padding: 16, backgroundColor: "rgba(0,0,0,0.45)", flex: 1, justifyContent: "flex-end" }}>
        {title ? <Text style={{ color: colors.text, fontSize: 20, fontWeight: "bold", marginBottom: 6 }}>{title}</Text> : null}
        {description ? <Text style={{ color: colors.textMuted, marginBottom: 12 }}>{description}</Text> : null}
        {children}
      </View>
    </ImageBackground>
  );
